...
@section('content')
    <div class="">
        ...
        <h2 class="text-center mt-3">Todos los Proveedores</h2>
        @if (session('success'))
    ...
    @can('proveedor_crear')
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <h6 class="modal-title small text-muted mb-3" id="importModalLabel2">Columnas: [Nombre, Direccion,
                            Celular]</h6>
        ...
        <div class="modal fade" id="addProveedorModal" tabindex="-1" aria-labelledby="addProveedorModalLabel"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title w-100 text-center" id="addProveedorModalLabel">Agregar nuevo proveedor</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
    ...
    <div class="d-flex justify-content-center mb-3 mt-n5">
        <form action="{{ route('proveedores.index') }}" method="GET" class="d-flex justify-content-center">
            <div class="input-group rounded d-flex justify-content-center">
                ...
                <div class="d-flex">
                    ...
                    <a href="{{ route('proveedores.index') }}" class="btn btn-secondary d-flex align-items-center gap-1">
                        <div style="display: inline-flex; align-items: center;">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="#ffffff"
                                style="width: 1.5rem; height: 1.5rem;"> <!-- Estilo para el icono -->
                                <path
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th scope="col" class="bg-light">Direccion</th>
            ...
            <tbody>
                @foreach ($proveedores as $proveedor)
                    <tr>
                        <td class="sticky-id bg-light">{{ $proveedor->id_proveedor }}</td>
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header text-white bg-danger">
                                                    ...
                                                    <h1 class="modal-title fs-5" id="exampleModalLabel">Eliminar proveedor
                                                    </h1>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <p><strong>¡Cuidado!</strong> ¿Estás seguro de que deseas
                                                        ...
                                                        y se
                                                        eliminarán todas las compras relacionadas con este proveedor.
                                                    </p>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center mt-4">
            <!-- Enlace "Anterior" -->
            @if ($proveedores->previousPageUrl())
                <li class="page-item">
                    <a class="page-link"
                        href="{{ $proveedores->previousPageUrl() . '&' . http_build_query(request()->except('page')) }}"
            ...
            @if ($start > 1)
                <li class="page-item">
                    <a class="page-link"
                        href="{{ $proveedores->url(1) . '&' . http_build_query(request()->except('page')) }}">1</a>
                </li>
                @if ($start > 2)
            ...
            @for ($i = $start; $i <= $end; $i++)
                <li class="page-item {{ $proveedores->currentPage() == $i ? 'active' : '' }}">
                    <a class="page-link"
                        href="{{ $proveedores->url($i) . '&' . http_build_query(request()->except('page')) }}">{{ $i }}</a>
                </li>
            @endfor
            ...
            @if ($end < $totalPages)
                ...
                <li class="page-item">
                    <a class="page-link"
                        href="{{ $proveedores->url($totalPages) . '&' . http_build_query(request()->except('page')) }}">{{ $totalPages }}</a>
                </li>
            @endif

            <!-- Enlace "Siguiente" -->
            @if ($proveedores->nextPageUrl())
                <li class="page-item">
                    <a class="page-link"
                        href="{{ $proveedores->nextPageUrl() . '&' . http_build_query(request()->except('page')) }}">Siguiente</a>
                </li>
            @else
                <li class="page-item disabled">
