...
@section('content')
    <div class="">
        ...
        <h2 class="text-center mt-3">Carreras</h2>
        @if (session('success'))
        ...
        <div class="d-flex justify-content-end my-3">
            @can('carrera_crear')
                ...
                <div class="modal fade" id="addCarreraModal" tabindex="-1" aria-labelledby="addCarreraModalLabel"
                    aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="addCarreraModalLabel">Agregar nueva carrera</h5>
                                ...
                                <div class="modal-body">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="codigo" name="codigo"
                                            placeholder="Codigo de la carrera" pattern="\d*" maxlength="10"
                                            oninput="this.value = this.value.replace(/[^0-9]/g, '').substring(0, 10)" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="nombre" class="form-label">Nombre</label>
                                        <input type="text" class="form-control" id="nombre" name="nombre"
                                            placeholder="Nombre de la carrera" pattern="[A-Za-zñÑáéíóúÁÉÍÓÚ .]+" maxlength="40"
                                            minlength="4"
                                            oninput="this.value = this.value.replace(/[^A-Za-zñÑáéíóúÁÉÍÓÚ .]/g, '').toUpperCase().substring(0, 40)"
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="bg-light align-middle">Código</th>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex justify-content-start align-items-center">
                                    @can('carrera_eliminar')
                                        <button type="button" class="btn btn-danger me-2" data-bs-toggle="modal"
                                            data-bs-target="#deleteCarreraModal{{ $carrera->id_carrera }}">
                                            Eliminar
                                        </button>
                                    @endcan
                                    <div style="width: 0.5rem;"></div>
                                    @can('carrera_actualizar')
                                        <button type="button" class="btn btn-info" data-bs-toggle="modal"
                                            data-bs-target="#editCarreraModal{{ $carrera->id_carrera }}" style="color: white">
                                            Editar
                                        </button>
                                    @endcan
                                    <div class="modal fade" id="editCarreraModal{{ $carrera->id_carrera }}"
                                        tabindex="-1" aria-labelledby="editCarreraModalLabel{{ $carrera->id_carrera }}"
                                        aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title"
                                                        id="editCarreraModalLabel{{ $carrera->id_carrera }}">Editar
                                                        carrera
                                                    </h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                                                </div>
                                                <form action="{{ route('carreras.update', $carrera->id_carrera) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="modal-body">
                                                        <div class="mb-3">
                                                            <label for="codigo{{ $carrera->id_carrera }}"
                                                                class="form-label">Código</label>
                                                            <input type="text" class="form-control"
                                                                id="codigo{{ $carrera->id_carrera }}" name="codigo"
                                                                value="{{ $carrera->codigo }}" pattern="\d*"
                                                                maxlength="10"
                                                                oninput="this.value = this.value.replace(/[^0-9]/g, '').substring(0, 10)"
                                                                required>
                                                        </div>
                                                        <div class="mb-3">
                                                            <label for="nombre{{ $carrera->id_carrera }}"
                                                                class="form-label">Nombre</label>
                                                            <input type="text" class="form-control"
                                                                id="nombre{{ $carrera->id_carrera }}" name="nombre"
                                                                value="{{ $carrera->nombre }}"
                                                                pattern="[A-Za-zñÑáéíóúÁÉÍÓÚ .]+" maxlength="40"
                                                                minlength="4"
                                                                oninput="this.value = this.value.replace(/[^A-Za-zñÑáéíóúÁÉÍÓÚ .]/g, '').toUpperCase().substring(0, 40)"
                                                                style="text-transform:uppercase;" required>
                                                        </div>
                                    ...
                                    <div class="modal fade" id="deleteCarreraModal{{ $carrera->id_carrera }}"
                                        tabindex="-1"
                                        aria-labelledby="deleteCarreraModalLabel{{ $carrera->id_carrera }}"
                                        aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header text-white bg-danger">
                                                    ...
                                                    <h1 class="modal-title fs-5" id="exampleModalLabel">Eliminar carrera
                                                    </h1>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <p><strong>¡Cuidado!</strong> ¿Estás seguro de que deseas
                                                        ...
                                                        Esta acción no se puede revertir y se eliminan de todos los
                                                        registros
                                                        de asignaciones relacionadas a esta carrera.
                                                    </p>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination justify-content-center mt-4">
                <!-- Enlace "Anterior" -->
                @if ($carreras->previousPageUrl())
                    <li class="page-item">
                        <a class="page-link" href="{{ $carreras->previousPageUrl() }}" tabindex="-1"
                ...
                <!-- Enlaces de páginas -->
                @foreach ($carreras->getUrlRange(1, $carreras->lastPage()) as $pagina => $url)
                    <li class="page-item {{ $carreras->currentPage() == $pagina ? 'active' : '' }}">
                        <a class="page-link" href="{{ $url }}">{{ $pagina }}</a>
                    </li>
                @endforeach

                <!-- Enlace "Siguiente" -->
                @if ($carreras->nextPageUrl())
                    <li class="page-item">
                        <a class="page-link" href="{{ $carreras->nextPageUrl() }}">Siguiente</a>
                    </li>
                @else
                    <li class="page-item disabled">
