        ...
        <div class="">
            <div class="d-flex justify-content-end my-3">
                @can('materia_crear')
                    ...
                    <button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#modalAgregarMateria">
                        + Agregar
                    </button>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <h6 class="modal-title small text-muted mb-3" id="importModalLabel2">Columnas: [Codigo,
                                        Nombre materia]</h6>
                                    <form action="{{ route('materias.import') }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form method="POST" action="{{ route('materias.store') }}">
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            <label for="codigo" class="form-label mb-0">Codigo</label>
                                            <input type="text" class="form-control" id="codigo" name="codigo"
                                                maxlength="10" placeholder="Codigo de materia"
                                                oninput="this.value = this.value.replace(/[^0-9]/g, '')" required>
                                        </div>
                                        <div class="mb-3">
                                            <label for="nombre_materia" class="form-label mb-0">Nombre de la materia</label>
                                            <input type="text" class="form-control" id="nombre_materia"
                                                name="nombre_materia" maxlength="40" placeholder="Nombre de la materia"
                                                oninput="this.value = this.value.replace(/[^a-zA-Z0-9\sÑñÁáÉéÍíÓóÚú.]/g, '').toUpperCase()"
                                                required>
                                        </div>
                                        <div id="labelsCarreras" class="row g-3 mb-3 d-none">
                                            <div class="col-md-9">
                                                <label for="id_carrera" class="form-label mb-0">Carrera</label>
                                            </div>
                                            <div class="col-md-2">
                                                <label for="nivel" class="form-label mb-0">Nivel</label>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="bg-light">Nombre materia</th>
                            <th scope="col" class="bg-light text-end">Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($materias as $materia)
                            <tr>
                                <td class="sticky-id bg-light col-sm-1"> {{ $materia->id_materia }}</td>
                                <td> {{ $materia->codigo }}</td>
                                <td> {{ $materia->nombre_materia }}</td>
                                <td class="d-flex justify-content-end align-items-center col">
                                    @can('materia_eliminar')
                                        <button class="btn btn-danger" type="button" data-bs-toggle="modal"
                                            data-bs-target="#deleteMateriaModal{{ $materia->id_materia }}">
                                            Eliminar
                                        </button>
                                    @endcan
                                    <div style="width: 0.5rem;"></div>
                                    @can('materia_actualizar')
                                        <button class="btn btn-info" type="button" data-bs-toggle="modal"
                                            data-bs-target="#editMateriaModal{{ $materia->id_materia }}">
                                            <span style="color: white;">Editar</span>
                                        </button>
                                    @endcan
                                    <div class="modal fade" id="editMateriaModal{{ $materia->id_materia }}"
                                        tabindex="-1" aria-labelledby="editMateriaModalLabel{{ $materia->id_materia }}"
                                        aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title"
                                                        id="editMateriaModalLabel{{ $materia->id_materia }}">
                                                        Editar
                                                        materia
                                                    </h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                ...
                                                <form action="{{ route('materias.update', $materia->id_materia) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="modal-body">
                                                        <div class="mb-3">
                                                            <label for="codigo{{ $materia->id_materia }}"
                                                                class="form-label">Codigo</label>
                                                            <input type="text" class="form-control"
                                                                id="codigo{{ $materia->id_materia }}" name="codigo"
                                                                maxlength="10"
                                                                oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                                                                value="{{ $materia->codigo }}" required>
                                                        </div>
                                                        <div class="mb-3">
                                                            <label for="nombre_materia{{ $materia->id_materia }}"
                                                                class="form-label">Nombre de la materia</label>
                                                            <input type="text" class="form-control"
                                                                id="nombre_materia{{ $materia->id_materia }}"
                                                                name="nombre_materia" maxlength="40"
                                                                oninput="this.value = this.value.replace(/[^a-zA-Z0-9\sÑñÁáÉéÍíÓóÚú.]/g,'').toUpperCase()"
                                                                value="{{ $materia->nombre_materia }}" required>
                                                        </div>
                                                        <div id="labelsCarreras{{ $materia->id_materia }}"
                                                            class="row g-3 mb-3 d-none">
                                                            <div class="col-md-9">
                                                                <label for="id_carrera"
                                                                    class="form-label mb-0">Carrera</label>
                                                            </div>
                                                            <div class="col-md-2">
                                                                <label for="nivel"
                                                                    class="form-label mb-0">Nivel</label>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="modal-footer">
                                    ...
                                    <div class="modal fade" id="deleteMateriaModal{{ $materia->id_materia }}"
                                        tabindex="-1"
                                        aria-labelledby="deleteMateriaModalLabel{{ $materia->id_materia }}"
                                        aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header text-white bg-danger">
                                                    ...
                                                    <h1 class="modal-title fs-5" id="exampleModalLabel">Eliminar materia
                                                    </h1>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <p><strong>¡Cuidado!</strong> ¿Estás seguro de que deseas
                                                        ...
                                                        de asignaciones que contienen a esta materia.
                                                    </p>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center mt-4">
                    <!-- Enlace "Anterior" -->
                    @if ($materias->previousPageUrl())
                        <li class="page-item">
                            <a class="page-link" href="{{ $materias->previousPageUrl() }}" tabindex="-1"
                    ...
                    <!-- Enlaces de páginas -->
                    @foreach ($materias->getUrlRange(1, $materias->lastPage()) as $pagina => $url)
                        <li class="page-item {{ $materias->currentPage() == $pagina ? 'active' : '' }}">
                            <a class="page-link" href="{{ $url }}">{{ $pagina }}</a>
                        </li>
                    @endforeach

                    <!-- Enlace "Siguiente" -->
                    @if ($materias->nextPageUrl())
                        <li class="page-item">
